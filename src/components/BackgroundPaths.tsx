  ...
  return (
    <div
      className={
        cn
          ? cn(
              "fixed inset-0 -z-10 h-full w-full bg-black",
              className
            )
          : `fixed inset-0 -z-10 h-full w-full bg-black ${className ?? ""}`
      }
    >
    ...
    >
      ...
      {circuits.map((path, idx) => (
        <motion.path
          key={`circuit-${idx}`}
          d={path}
          stroke="url(#circuitGradientDark)"
          strokeWidth="2"
          fill="none"
          filter="url(#circuitGlowDark)"
          strokeLinecap="round"
          opacity="0.3"
