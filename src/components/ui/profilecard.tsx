...
export default function ProfileCard({
  ...
  const renderSkillIcon = (skill: Skill) => {
    ...
    if (React.isValidElement(skill.icon)) {
      return React.cloneElement(skill.icon as React.ReactElement<any>, { className: "w-5 h-5" });
    }
  ...
  return (
    <div className="flex items-center justify-center">
      <motion.div 
        ref={cardRef} 
        className={`relative w-[17rem] h-[21.2rem] rounded-[15px] overflow-hidden shadow-[0_4px_10px_rgba(0,0,0,0.5)] bg-gray-900 border border-white/10 ${spotlight && isRevealed ? 'group' : ''}`} 
        onMouseMove={handleMouseMove} 
        onMouseLeave={handleMouseLeave} 
        style={isRevealed ? { rotateX, rotateY, translateX, translateY, transformStyle: "preserve-3d", transition: "transform 0.1s ease-out" } : {}}
      >
        ...
        <motion.div 
          ...
          animate={{ width: isImageShrunken ? "6rem" : "100%", height: isImageShrunken ? "8rem" : "100%", top: isImageShrunken ? "4.5rem" : 0, left: isImageShrunken ? "1rem" : 0, borderRadius: isImageShrunken ? "0.5rem" : "0px" }} 
          transition={{ duration: 0.4, ease: "easeInOut" }} 
          className="absolute overflow-hidden"
        >
          <Image src={img} alt={name} fill style={{ objectFit: "cover" }} />
          <motion.div 
            className="absolute top-2 right-2 w-[1.6875rem] h-[1.8125rem] flex items-center justify-center bg-gray-800 border border-white/20 rounded shadow cursor-grab active:cursor-grabbing z-10" 
          ...
          >
            <motion.div style={{ rotate: arrowRotation }}>
              <ArrowBigLeft className="w-4 h-4 text-white pointer-events-none" />
            </motion.div>
          </motion.div>
        </motion.div>
        ...
        <AnimatePresence>
          {isRevealed && (
            ...
            >
              <motion.div 
                className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center bg-gray-800 border border-white/20 rounded-full cursor-pointer z-20" 
                onClick={resetCard} 
              ...
              <div className="p-4">
                <h1 className="text-xl font-bold tracking-wider text-white">{name}</h1>
                <p className="text-sm tracking-wider text-gray-300">{position}</p>
              </div>
              ...
              <div className="absolute top-[4.5rem] left-[8rem]">
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill, index) => (
                    <div 
                      key={index} 
                      className={`bg-gray-700 border border-gray-600 rounded-md p-1 flex items-center justify-center w-8 h-8 ${spotlight ? 'group relative overflow-hidden' : ''}`} 
                      title={skill.name} 
                    ...
                    >
                      ...
                      <div className="relative z-10 text-white">{renderSkillIcon(skill)}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                {socialLinks.map((social, index) => {
                  const renderIcon = () => {
                    if (typeof social.icon === 'string') {
                      ...
                      return React.cloneElement(social.icon as React.ReactElement<any>, { className: "w-5 h-5", width: 20, height: 20 });
                  ...
                  return (
                    <Link key={index} href={social.url} target="_blank" rel="noreferrer">
                      <div 
                        className={`w-5 h-5 relative flex items-center justify-center ${spotlight ? 'group overflow-hidden rounded' : ''}`} 
                        title={social.name} 
                        onMouseMove={spotlight ? (e) => {
                          const rect = e.currentTarget.getBoundingClientRect();
                          const x = e.clientX - rect.left;
        ...
        {!isRevealed && !isImageShrunken && (
          <motion.div 
            ...
            initial={{ opacity: 0 }} 
            animate={{ opacity: [0, 0.8, 0], transition: { repeat: Infinity, duration: 2, repeatDelay: 1 } }}
          >
            Drag →
          </motion.div>
        )}
